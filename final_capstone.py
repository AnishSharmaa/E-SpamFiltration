...
path = "/home/flask/flask_app/SpamFilterMachineLearning-master/data/"
...
emails_train = pd.concat([
    pd.DataFrame({"review":spam_train,
                  "label":1}),
    pd.DataFrame({"review":nonspam_train,
                  "label":0}),
],
...
for sentence in emails_train['review']:
    X_train.append(sent_vectorizer(
        sentence,
        model_train_word2vec))

X_test = []
for sentence in emails_test['review']:
    X_test.append(sent_vectorizer(
        sentence,
        model_train_word2vec))
